...
from giles.games.lost_cities import lost_cities_card
...
from lost_cities_card import LostCitiesCard
from lost_cities_card import card_to_str, get_color_code, hand_to_str, value_to_str, sorted_hand, str_to_card, str_to_suit
from lost_cities_card import DEFAULT_SUITS, YELLOW, BLUE, WHITE, GREEN, RED, CYAN, MAGENTA
from lost_cities_card import AGREEMENT, SHORT, LONG
...
class LostCities(Game):
    """A Lost Cities game table implementation.  Invented in 1999 by Reiner
    Knizia.
    """

    def __init__(self, server, table_name):

        super(LostCities, self).__init__(server, table_name)

        self.game_display_name = "Lost Cities"
        self.game_name = "lostcities"
        self.seats = [
            Seat("Left"),
            Seat("Right")
        ]
        ...
        self.prefix = "(^RLost Cities^~): "
        self.log_prefix = "%s/%s: " % (self.table_display_name, self.game_display_name)

        # LostCities-specific stuff.
        self.suit_count = 5
    ...
    def init_hand(self):
        ...
        for suit in suit_list:
            for rank in NUMERICAL_RANKS:
                self.draw_pile.add(LostCitiesCard(rank, suit))

            # Add as many agreements as requested.
            for agreement in range(self.agreement_count):
                self.draw_pile.add(LostCitiesCard(AGREEMENT, suit))
    ...
    def show_help(self, player):
        ...
        super(LostCities, self).show_help(player)
        player.tell_cc("\nLOST CITIES SETUP PHASE:\n\n")
